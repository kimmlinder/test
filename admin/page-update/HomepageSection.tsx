  ...
  const fetchData = async () => {
    try {
      const [settingsRes, projectsRes] = await Promise.all([
        ...
        supabase.from('projects').select('id, title, category, image_url, published').order('title')
      ]);
        ...
        <TabsContent value="projects" className="space-y-6">
          <div className="bg-card border border-border rounded-2xl p-6">
            ...
            <p className="text-muted-foreground text-sm mb-6">
              Select up to 6 projects to feature on the homepage.
            </p>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-1">
                          {index > 0 && (
                            <Button variant="ghost" size="sm" onClick={() => moveProject(index, index - 1)}>↑</Button>
                          )}
                          {index < selectedProjectIds.length - 1 && (
                            <Button variant="ghost" size="sm" onClick={() => moveProject(index, index + 1)}>↓</Button>
                          )}
                          <Button variant="ghost" size="sm" onClick={() => toggleProjectSelection(id)}>
